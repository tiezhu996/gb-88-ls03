...
- 预置项目：「用户管理系统」
- 7 个示例 API 接口（含同路径多份 Mock：固定数据、按角色返回、已停用各一份）
- 10 条请求日志记录
...
- 自定义响应头
- 响应延迟模拟
- 同一路径可配置多份 Mock 数据，通过**优先级**决定命中顺序：数字越大越先命中，数字相同按创建时间早的先命中
- 每个接口支持**启用/停用开关**，停用的接口不参与匹配，也不会成为命中来源
- 接口和条件规则支持命名，便于在日志中识别命中来源
...
- 明确标注每次请求**命中的接口和条件规则**（接口名称、路径、优先级、规则名称）
- 未命中任何启用接口的请求也照记一条，标记为「未命中」且不关联任何接口
- 支持日志搜索和清空
